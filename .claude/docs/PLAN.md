...
**Status:** M0–M4 done (`0.1.0`) · **Target first release:** `1.0.0` · **npm name:** `namespaced-storage` (verified available)
...
- **Size (`size-limit`, minified + brotli).** Budgeted per milestone rather than once up front, so
  each milestone has to justify its own weight: after M4, 4.5 kB (actual 4.17; 4.77 with `t.*`). 1.0 target < 6 kB · `sideEffects: false`
