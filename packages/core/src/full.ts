...
/** Everything above level 1, wired in. `namespaced-storage/minimal` wires in none of it. */
const FEATURES = {
  typing: resolveTyping,
  migration: { assert: assertMigrationOptions, run: runMigration },
  devtools: registerDevStore,
};
...
function factory(adapter: () => SyncAdapter): Factory {
  return makeFactory(adapter, FEATURES) as Factory;
}
